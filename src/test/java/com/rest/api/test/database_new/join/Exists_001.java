...
public class Exists_001 {

    public static void main(String args[]){ // SQL statement returns TRUE and lists the suppliers with a product price less than 30
        try{
            ...
            ResultSet rs = stmt.executeQuery("SELECT customerNumber\n" +
                    "    FROM Orders\n" +
                    "    WHERE EXISTS (SELECT productCode FROM orderdetails WHERE orderdetails.orderNumber = Orders.orderNumber AND priceEach < 30)");
